...
def L_O_kinetic (A : Fin 8 → Fin 4 → ℝ) : ℝ :=
...
def L_O_phi_coupling (A : Fin 8 → Fin 4 → ℝ) (φ_val : ℝ) : ℝ :=
  alpha * Real.log (φ_val + 1) * ∑ ν : Fin 4, (∂φ ν) * A 0 ν

/-- **O-Maxwell Lagrangian density** (scalar at one "point"; sum over O and spacetime). -/
def L_O_Maxwell (A : Fin 8 → Fin 4 → ℝ) (φ_val : ℝ) : ℝ :=
  L_O_kinetic A + 4 * π * L_O_source A + L_O_phi_coupling A φ_val

/-- **O-Maxwell action** S_O = L_O (integral replaced by single cell / sum). -/
def action_O_Maxwell (A : Fin 8 → Fin 4 → ℝ) (φ_val : ℝ) : ℝ :=
  L_O_Maxwell A φ_val

/-- **Euler–Lagrange** from varying A(a,ν): δS_O/δA(a,ν). EL_O = 0 ⟺ div F = 4π J + φ term (emergent equation). -/
def EL_O (A : Fin 8 → Fin 4 → ℝ) (φ_val : ℝ) (a : Fin 8) (ν : Fin 4) : ℝ :=
  (∑ μ : Fin 4, F_from_A A a μ ν) - 4 * π * J_O a ν
...
def S_HQVM_grav (φ rho_m rho_r : ℝ) : ℝ :=
...
theorem S_HQVM_grav_zero_iff_Friedmann (φ rho_m rho_r : ℝ) :
  ...
  unfold S_HQVM_grav HQVM_Friedmann_eq G_eff rho_total H_of_phi
...
def action_total (A : Fin 8 → Fin 4 → ℝ) (φ_val rho_m rho_r : ℝ) : ℝ :=
...
theorem equations_from_action (φ rho_m rho_r : ℝ) (hφ : 0 ≤ φ) :
    (S_HQVM_grav φ rho_m rho_r = 0 ↔ HQVM_Friedmann_eq φ rho_m rho_r) ∧
    (∀ a ν, EL_O A_O (φ + 1) a ν = (∑ μ : Fin 4, F_from_A A_O a μ ν) - 4 * π * J_O a ν -
      (if a = 0 then alpha * Real.log (φ + 2) * ∂φ ν else 0)) := by
  refine ⟨S_HQVM_grav_zero_iff_Friedmann φ rho_m rho_r, fun a ν => ?_⟩
  unfold EL_O A_O F_from_A J_O
  simp only [sub_zero, zero_mul, Finset.sum_const_zero, sub_self]
